...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load(r"D:\Desktop\A\tea_tf2.3\tea_tf2.3-master\new_data\train",
                                              r"D:\Desktop\A\tea_tf2.3\tea_tf2.3-master\new_data\val", 224, 224, 16)

    model = tf.keras.models.load_model("models/mobilenet_tea.h5")

    # 保存模型为TensorFlow Serving所需的SavedModel格式
    model.save('local_models/mobilenet_tea', save_format='tf')
...
def test_cnn():
    ...
    model = tf.keras.models.load_model("models/cnn_tea.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
