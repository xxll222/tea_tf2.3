...
def data_load(data_dir, test_data_dir, img_height, img_width, batch_size):
    ...
    datagen = ImageDataGenerator(
        ...
        width_shift_range=0.1,  # 水平平移的幅度
        height_shift_range=0.1,  # 垂直平移的幅度
        shear_range=0.1,  # 剪切强度
        zoom_range=0.1,  # 随机缩放的比例
        horizontal_flip=True,  # 是否进行水平翻转
        fill_mode='nearest'  # 处理边界时的方式
    )
    ...
    # 获取训练集的类别名
    class_names = [str(key) for key in train_ds.class_indices.keys()]
...
def model_load(IMG_SHAPE=(224, 224, 3), class_num=8):
    # 加载预训练的mobilenet模型
...
def train(epochs):
    ...
    # 定义 TensorBoard 日志目录
    log_dir = "logs/fine_tuned_model"

    # 创建 TensorBoard 回调对象
    tb_callback = TensorBoard(log_dir=log_dir, histogram_freq=0, write_graph=True, update_freq='epoch')
